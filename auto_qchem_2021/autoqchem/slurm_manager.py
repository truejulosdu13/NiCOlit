...
class slurm_manager(object):
    ...
    def connect(self) -> None:
        ...
        if create_new_connection:
            ...
            #self.connection.set_missing_host_key_policy('forward_agent', 'forward_remote')
            #self.connection.connect('hermite', 22, {self.user}, {self.password})

            #c = fabric.Connection('hermite')
            #c.client = client
            #c.transport = client.get_transport()
            #run(f"mkdir -p {self.remote_dir}")
            
            # added by jules :
            #print
            #logger.info(f"Creating connection to hermite as {self.user}")
            #new_connection = ssh_connect("hermite", self.user)
            #new_connection.run(f"mkdir -p {self.remote_dir}")
