...
from aqc_utils.descriptor_functions import *
...
class gaussian_log_extractor(object):
    ...
    def _split_parts(self) -> None:
        ...
        # log_parts = re.split("\n\s-+\n\s#\s", self.log)[1:] version g16
        log_parts = re.split("Entering Link 1", self.log)[1:]
        self.parts = {}
        # added
        name_parts = ['opt', 'freq', 'TD']
        #for p in log_parts:
        for i,j in enumerate(log_parts):
